...
use App\Http\Integrations\RaidHelper\Data\Events\SignUpData;
use App\Models\User;
...
class EventDataTest extends TestCase
{
    ...
    {
        $event = EventData::from($this->listingPayload());
    ...
    {
        $event = EventData::from($this->detailPayload());
    ...
    {
        $event = EventData::from($this->listingPayload(['softresId' => 'softres-xyz']));
    ...
    {
        $event = EventData::from($this->listingPayload(['imageUrl' => 'https://example.com/raid.png']));
    ...
    {
        $event = EventData::from($this->listingPayload(['signUpCount' => 12]));
    ...
    public function it_stores_sign_ups_when_provided(): void
    {
        ...
        $this->assertInstanceOf(SignUpData::class, $event->signUps[0]);
    ...
    {
        $event = EventData::from($this->detailPayload(['advancedSettings' => ['duration' => 180]]));

        $this->assertInstanceOf(EventAdvancedSettingsData::class, $event->advancedSettings);
    ...
    public function it_hydrates_classes_as_event_class_instances(): void
    {
        ...
        $this->assertInstanceOf(EventClassData::class, $event->classes[0]);
    ...
    public function it_hydrates_roles_as_event_role_instances(): void
    {
        ...
        $this->assertInstanceOf(EventRoleData::class, $event->roles[0]);
    ...
    {
        $event = EventData::from($this->listingPayload(['leaderId' => 'nonexistent-discord-id']));
    ...
    {
        $array = EventData::from($this->listingPayload())->toArray();
    ...
    {
        $array = EventData::from($this->detailPayload())->toArray();
    ...
    {
        $event = EventData::from($this->listingPayload(['scheduledId' => '902879335676006421-1']));
    ...
    {
        $event = EventData::from($this->listingPayload(['displayTitle' => 'Karazhan']));
    ...
    {
        $event = EventData::from($this->listingPayload(['announcements' => []]));
    ...
    #[Test]
    public function it_accepts_missing_description(): void
    {
        $payload = $this->listingPayload();
        unset($payload['description']);

        $event = EventData::from($payload);
