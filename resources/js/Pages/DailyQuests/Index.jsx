...
function QuestsSkeleton() {
    return (
        <div className="space-y-6">
            {[...Array(5)].map((_, i) => (
                <div key={i} className="animate-pulse rounded-lg bg-ground-800 p-4">
                    <div className="mb-3 h-5 w-32 rounded bg-ground-700" />
                    <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded bg-ground-700" />
                        <div className="h-4 w-48 rounded bg-ground-700" />
                    </div>
                </div>
            ))}
        </div>
...
function QuestCard({ quest }) {
    ...
    return (
        <div className="mb-8 w-full rounded-lg bg-ground-800/50 p-6">
            <h3 className="mb-1 text-2xl font-bold text-ink-400">
            ...
            <div>
                ...
                <div className="flex flex-wrap gap-4">
                    {quest.rewards.map((reward) => (
                        <QuestReward key={reward.id} reward={reward} />
                    ))}
                </div>
            </div>
        </div>
