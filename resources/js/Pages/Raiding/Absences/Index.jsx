...
function PlannedAbsencesSkeleton() {
    return (
        <div className="animate-pulse space-y-4">
            {[...Array(3)].map((_, i) => (
                <div key={i} className="h-16 rounded bg-surface/50" />
            ))}
        </div>
    );
}
                    ...
                    <div className="mb-4 flex flex-row justify-end">
                        <Can permission="manage-planned-absences">
                            <Link
                                ...
                                className="mt-3 inline-flex items-center rounded-md border border-transparent bg-accent px-4 py-2 text-sm font-semibold tracking-wide text-white transition duration-150 ease-in-out hover:bg-accent-hover focus:bg-accent-hover focus:outline-hidden focus:ring-2 focus:ring-focus-ring focus:ring-offset-2 active:bg-accent-active md:mt-0"
                            >
                        ...
                        <div className="flex flex-col gap-6">
                            {Object.entries(grouped).map(([characterName, absences]) => (
                                <div key={characterName}>
                                    <h2 className="mb-3 text-lg font-semibold text-body">{characterName}</h2>
