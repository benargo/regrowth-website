    ...
    return (
        <Tooltip body="View on Warcraft Logs" position="top">
            <a
                ...
                className="inline-flex items-center gap-2 rounded border border-camel-600 px-4 py-2 text-sm text-gray-200 transition-colors hover:bg-camel-600/20"
            >
        ...
        <div className="overflow-x-auto">
            <table className="w-full border-collapse">
                <thead className="border-b border-camel-600">
                    <tr>
                        <th className="px-4 py-3 text-left text-sm font-semibold text-camel-500">Character</th>
                        <th className="px-4 py-3 text-left text-sm font-semibold text-camel-500">Rank</th>
                        <th className="px-4 py-3 text-left text-sm font-semibold text-camel-500">Class</th>
                        <th className="px-4 py-3 text-left text-sm font-semibold text-camel-500">Race</th>
                        <Can permission="view-attendance">
                            <th className="px-4 py-3 text-left text-sm font-semibold text-camel-500">Attendance</th>
                        </Can>
                    </tr>
                </thead>
                <tbody className="divide-y divide-forever-600">
                    {characters.map((character) => (
                        <tr key={character.id} className="transition-colors hover:bg-forever-800/50">
                            <td className="px-4 py-3">
                                <span className="text-sm font-medium text-white">{character.name}</span>
                                {character.is_main && (
                                    <span className="ml-2 rounded bg-camel-600/20 px-1.5 py-0.5 text-xs text-camel-400">
                                        Main
                                    </span>
                                )}
                            </td>
                            ...
                            <Can permission="view-attendance">
                                <td className="px-4 py-3">
                                    <Link
                                        ...
                                        className="text-sm text-camel-400 hover:text-camel-300 hover:underline"
                                    >
                                        View attendance
                                    </Link>
                                </td>
    ...
    return (
        <Master title={data.title}>
            ...
            <PageContainer>
                ...
                <div className="mb-8 rounded border border-camel-600/30 bg-forever-800/50 p-4">
                ...
                <div className="rounded border border-camel-600/30">
