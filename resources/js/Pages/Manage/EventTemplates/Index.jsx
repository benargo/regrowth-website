...
function RaidBadge({ raid }) {
    return <span className="rounded bg-camel-600/20 px-2 py-0.5 text-xs text-camel-400">{raid.name}</span>;
}
...
function TemplateCard({ template, onDeleteClick }) {
    ...
    return (
        <div className="flex flex-col gap-3 rounded border border-camel-600/40 bg-forever-800/40 p-4">
            ...
            <div className="flex gap-2">
                <Link
                    ...
                    className="flex items-center gap-1 rounded border border-camel-600 px-3 py-1 text-sm text-camel-400 transition-colors hover:bg-camel-600/20"
                >
            ...
            <PageContainer>
                <div className="mb-8 flex items-center justify-between">
                    ...
                    <Link
                        ...
                        className="flex items-center gap-2 rounded bg-camel-600 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-camel-500"
                    >
                ...
                {raidGroups.length === 0 ? (
                    ...
                    <div className="flex flex-col gap-12">
                        {raidGroups.map(({ raid, templates: raidTemplates }) => (
                            <section key={raid.id}>
                                <h2 className="mb-4 text-xl font-semibold text-camel-400">{raid.name}</h2>
