...
export default function Create({ raids }) {
    ...
    function handleSubmit(e) {
        e.preventDefault();
        post(route("management.event-templates.store"));
    }
                        ...
                        {raids.length === 0 ? (
                            ...
                            <div className="flex flex-col gap-2">
                                {raids.map((raid) => (
                                    <label
                                        key={raid.id}
                                        className="flex cursor-pointer items-center gap-3 rounded border border-gray-700 px-3 py-2 transition-colors hover:border-line/50 hover:bg-accent/10"
                                    >
                                        <input
                                            type="checkbox"
