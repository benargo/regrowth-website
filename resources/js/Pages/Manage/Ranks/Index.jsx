...
function SortableRankItem({ rank, index, onNameChange, isSavingName }) {
    ...
    return (
        <div
            ref={setNodeRef}
            style={style}
            className="flex items-center justify-between border-b border-camel-600 bg-forever-900 px-4 py-2 last:border-b-0"
        >
            ...
            {isEditing ? (
                <input
                    ...
                    className="mr-2 flex-auto rounded border border-camel-600 bg-forever-800 px-2 py-0.5 text-left text-white focus:outline-hidden focus:ring-1 focus:ring-camel-500"
                />
            ) : (
                <span
                    className="mr-2 flex-auto cursor-text text-left transition-colors hover:text-camel-400"
                    onClick={handleNameClick}
                    title="Click to edit"
                >
                    {rank.name}
                    <span className={isSavingName === rank.id ? "inline" : "hidden"}>
                        <Icon icon="spinner" style="solid" className="fa-spin ml-2 text-xs text-camel-400" />
                    </span>
                </span>
            )}
...
function RankOverlay({ rank, index }) {
    ...
    return (
        <div className="flex cursor-grabbing items-center justify-between rounded border border-camel-600 bg-forever-900 px-4 py-2 shadow-lg">
            <span className="mr-2 w-6 flex-initial text-right">{index + 1}.</span>
...
function NewRankInput({ nextPosition, onSave, isSaving }) {
    ...
    return (
        <div className="flex items-center justify-between border-t border-camel-600 bg-forever-800/50 px-4 py-2">
            ...
            {isSaving && (
                <span className="flex-none">
                    <Icon icon="spinner" style="solid" className="fa-spin text-xs text-camel-400" />
                </span>
            )}
        </div>
    );
        ...
        <Master title="Manage Guild Ranks">
            ...
            <PageContainer>
                ...
                <div className="mt-6 w-64">
                    {ranks.length === 0 ? (
                        <div className="flex flex-col rounded border border-camel-600">
