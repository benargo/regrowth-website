...
function PriorityIcon({ icon, title, size = "h-5 w-5" }) {
    if (!icon) {
        return <span className={`${size} bg-forever-700 shrink-0 rounded`} />;
    }
...
function DesktopTable({ phases, rows, columnMax, phaseTotals }) {
    return (
        <div className="hidden overflow-x-auto rounded border border-camel-600/30 md:block">
            <table className="w-full min-w-max border-collapse">
                <thead className="bg-forever-900 sticky top-0 border-b border-camel-600/30">
                    <tr>
                        <th className="px-4 py-3 text-left text-sm font-semibold text-camel-500">Priority</th>
                        {phases.map((phase) => (
                            <th key={phase.id} className="px-3 py-3 text-center text-sm font-semibold text-camel-500">
                                {phase.name}
                            </th>
                        ))}
                    </tr>
                </thead>
                <tbody className="divide-forever-600 divide-y">
                    {rows.map((row) =>
                        row.kind === "class" ? (
                            <Fragment key={`class-${row.id}`}>
                                ...
                                {row.children.map((child) => (
                                    <tr key={`priority-${child.id}`} className="hover:bg-forever-800/50">
                            ...
                            <tr key={`priority-${row.id}`} className="hover:bg-forever-800/50">
                                <td className="px-4 py-2">
                                    <div className="flex items-center gap-2">
                ...
                <tfoot className="bg-forever-900/60 border-t border-camel-600/30">
                    <tr>
                        <td className="px-4 py-3 text-sm font-semibold text-camel-500">Total</td>
                        {phases.map((phase) => (
                            <td
                                key={phase.id}
                                className="px-3 py-3 text-center text-sm font-semibold text-camel-400 tabular-nums"
                            >
...
function MobileTable({ phases, rows, selectedPhaseId, onSelectPhase, phaseTotals }) {
    ...
    return (
        <div className="flex flex-col gap-4 md:hidden">
            <div className="flex overflow-x-auto rounded border border-camel-600/30">
                {phases.map((p) => (
                    <button
                        key={p.id}
                        type="button"
                        ...
                        className={`flex-1 px-3 py-2 text-sm font-medium whitespace-nowrap transition-colors ${
                            p.id === phase?.id
                                ? "bg-camel-600/20 text-camel-400"
                                : "hover:bg-forever-800/50 text-gray-400"
                        }`}
                    >
                        {p.name}
                    </button>
            ...
            {phase && (
                <div className="divide-forever-600 divide-y rounded border border-camel-600/30">
                    {rows.map((row) =>
                        row.kind === "class" ? (
                            <div key={`class-${row.id}`}>
                                ...
                                >
                                    ...
                                    <span className="text-sm font-semibold text-camel-400 tabular-nums">
                    ...
                    <div className="bg-forever-900/60 flex items-center justify-between px-4 py-3">
                        <span className="text-sm font-semibold text-camel-500">Total</span>
                        <span className="text-sm font-semibold text-camel-400 tabular-nums">
                            {phaseTotals[phase.id] ?? 0}
                        </span>
                    </div>
...
function TableSkeleton() {
    ...
    return (
        <div className="animate-pulse overflow-x-auto rounded border border-camel-600/30">
            <table className="w-full min-w-max border-collapse">
                <thead className="border-b border-camel-600/30">
                    <tr>
                        <th className="px-4 py-3">
                            <div className="bg-forever-700 h-4 w-32 rounded" />
                        </th>
                        {fakeCols.map((_, i) => (
                            <th key={i} className="px-3 py-3">
                                <div className="bg-forever-700 mx-auto h-4 w-10 rounded" />
                            </th>
                        ))}
                    </tr>
                </thead>
                <tbody className="divide-forever-600 divide-y">
                    {fakeRows.map((_, i) => (
                        <tr key={i}>
                            <td className="px-4 py-2">
                                <div className="bg-forever-700 h-4 w-28 rounded" />
                            </td>
                            {fakeCols.map((_, j) => (
                                <td key={j} className="px-3 py-2">
                                    <div className="bg-forever-700 mx-auto h-4 w-4 rounded" />
                                </td>
                            ))}
                        </tr>
                    ))}
...
function EmptyState() {
    return (
        <div className="flex flex-col items-center gap-3 rounded border border-camel-600/30 py-16 text-center">
            <Icon icon="sack" style="light" className="text-4xl text-camel-500/70" />
