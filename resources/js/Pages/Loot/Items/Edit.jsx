...
function DraggablePriorityItem({ priority, onRemove }) {
    ...
    return (
        <div
            ref={setNodeRef}
            style={style}
            className="min-w-50 relative flex cursor-grab items-center justify-center gap-2 rounded-md border border-primary bg-forever-800 p-6"
            {...attributes}
            {...listeners}
        >
...
function PriorityOverlay({ priority }) {
    ...
    return (
        <div className="flex w-60 cursor-grabbing items-center justify-center gap-2 rounded-md border border-primary bg-forever-800 p-6 shadow-lg">
            {priority.media && <img src={priority.media} alt="" className="h-6 w-6 rounded-xs" />}
            <span>{priority.title}</span>
        </div>
    );
...
function DroppableWeightRow({ weight, children, onAddClick }) {
    ...
    return (
        <div
            ref={setNodeRef}
            className={`flex min-h-24 items-center justify-center transition-colors ${isOver ? "bg-camel-900/30" : ""}`}
        >
            ...
            <div className="ml-4 flex w-full flex-wrap items-center justify-center gap-4 py-4">
                ...
                <button
                    ...
                    className="flex h-12 w-12 items-center justify-center rounded-full bg-camel-600 text-white transition-colors hover:bg-camel-700"
                >
...
function InsertWeightZone({ afterWeight, onDrop, onAddClick }) {
    ...
    return (
        ...
        >
            <div className="my-4 text-center text-4xl font-bold text-camel-600">
            ...
            >
                <button
                    ...
                    className={`flex h-10 w-10 items-center justify-center rounded-full transition-colors ${
                        isOver ? "bg-camel-500 text-white" : "bg-camel-600 text-white hover:bg-camel-700"
                    }`}
                >
                    <Icon icon="plus" style="solid" />
                </button>
    ...
    return (
        <div
            ...
            className={`flex items-center justify-center rounded-lg border-2 border-dashed py-8 transition-colors ${
                isOver ? "border-camel-500 bg-camel-900/20" : "border-camel-600/30"
            }`}
        >
            <button
                type="button"
...
function PriorityPickerModal({ isOpen, onClose, priorities, onSelect }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
            <div
                className="max-h-[80vh] max-w-6xl overflow-y-auto rounded-lg border border-primary bg-forever-900 p-6"
            ...
            >
                ...
                {Object.entries(groupedPriorities).map(([type, typePriorities]) => (
                    <div key={type} className="mb-4">
                        <h4 className="mb-2 text-sm font-semibold uppercase text-camel-500">{type}</h4>
                        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5">
                            {typePriorities.map((priority) => (
                                <button
                                    ...
                                    className="flex items-center gap-2 rounded-md border border-primary/50 bg-forever-800 p-3 text-left transition-colors hover:bg-forever-700"
                                >
                                    {priority.media && (
        ...
        return (
            <div className="py-8 text-center">
                ...
                <button
                    ...
                    className="inline-flex items-center gap-2 rounded-md bg-camel-600 px-4 py-2 text-white transition-colors hover:bg-camel-700"
                >
                    <Icon icon="plus" style="solid" />
                        ...
                        <DroppableWeightRow weight={weight} onAddClick={handleAddToWeight}>
                            {groupedPriorities[weight].map((priority, index) => (
                                <div key={priority.id} className="flex items-center">
                                    {index > 0 && (
                                        <div className="mx-2 w-12 flex-none items-center text-center text-2xl font-bold text-camel-600">
                    ...
                    <div className="mb-4 flex flex-col justify-between gap-4 md:flex-row">
                        <div className="flex items-center justify-between gap-4 md:order-2">
                            <button
                                ...
                                className="hover:bg-forever-700 focus:bg-forever-700 focus:ring-forever-500 active:bg-forever-800 inline-flex items-center rounded-md border border-transparent px-4 py-2 text-sm font-semibold tracking-wide text-white transition duration-150 ease-in-out focus:ring-2 focus:ring-offset-2 focus:outline-hidden"
                            >
