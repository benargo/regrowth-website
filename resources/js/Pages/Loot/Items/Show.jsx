...
function PriorityItem({ priority }) {
    return (
        <div className="border-primary bg-ground-800 flex w-auto items-center justify-center gap-2 rounded-md border px-4 py-3">
...
function PriorityDisplay({ priorities }) {
    ...
    const rowBgs = ["bg-ground-900/60", "bg-ground-800/60", "bg-ground-700/60", "bg-ground-600/60"];
    ...
    return (
        <div className="overflow-hidden rounded-md text-lg">
            {weights.map((weight, weightIndex) => (
                <div key={weight}>
                    {weightIndex > 0 && (
                        <div className="flex justify-center py-1 text-2xl text-ink-700/60">
                    ...
                    >
                        <div className="flex h-8 w-8 flex-none items-center justify-center rounded-full border border-ink-600 text-lg font-bold text-ink-600">
                            {weightIndex + 1}
                        </div>
                                ...
                                <div key={`priority-${priority.id}`} className="flex items-center gap-2">
                                    {index > 0 && (
                                        <Icon
                                            ...
                                            icon="equals"
                                            style="solid"
                                            className="flex-none text-2xl font-bold text-ink-600"
                                        />
                                    )}
    ...
    return (
        <Master title={item.data.name}>
            ...
            <PageContainer>
                ...
                <div className="border-ink-600 my-8 border-t" />
