...
function StatSegment({ icon, label, value, index }) {
    return (
        <div
            className="animate-fade-in-up flex min-w-32 flex-1 items-center gap-3 px-5 py-4 transition-colors duration-200 hover:bg-ink-600/5"
...
function StatsRow({ stats }) {
    ...
    return (
        <div className="bg-ground-900/40 flex flex-col divide-y divide-ink-600/20 rounded-lg border border-ink-600/30 sm:flex-row sm:flex-wrap sm:divide-x sm:divide-y-0">
            {items.map((item, index) => (
                <StatSegment key={item.label} index={index} {...item} />
            ))}
            <Link
                href={route("loot.comments")}
                className="animate-fade-in-up group flex flex-1 items-center gap-3 px-5 py-4 transition-colors duration-200 hover:bg-ink-600/10"
                style={{ animationDelay: `${items.length * 60}ms` }}
            >
                <Icon
                    icon="external-link"
                    style="light"
                    className="flex size-6 items-center justify-center text-xl text-ink-400"
                />
                <span className="flex items-center gap-1.5 text-sm font-semibold text-white">
                    ...
                    <Icon
                        icon="chevron-right"
                        style="solid"
                        className="flex size-3 items-center justify-center text-xs text-ink-400 transition-transform duration-200 group-hover:translate-x-0.5"
                    />
                </span>
            </Link>
