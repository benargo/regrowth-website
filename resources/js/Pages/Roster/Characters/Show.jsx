...
function ReportsSkeleton() {
    return (
        <div className="animate-pulse space-y-2">
            {[...Array(5)].map((_, i) => (
                <div key={i} className="h-12 rounded bg-forever-800/50" />
            ))}
        </div>
    );
}
...
function ReportRow({ report }) {
    ...
    return (
        <Link
            ...
            className="flex items-center justify-between rounded border border-forever-600 bg-forever-800/40 px-4 py-3 transition-colors hover:border-camel-600/40 hover:bg-forever-800"
        >
...
function SectionHeading({ children }) {
    return (
        <h2 className="mb-4 flex items-center gap-3 text-xs font-bold uppercase tracking-[0.15em] text-camel-500/70">
            <span>{children}</span>
            <span className="h-px flex-1 bg-camel-600/20" />
        </h2>
    );
}
                ...
                <div className="mb-8 flex flex-wrap items-center gap-4">
                    {character.portrait_url ? (
                        <img
                            ...
                            alt={character.name}
                            className="h-20 w-20 rounded-xl border border-camel-600/30 shadow-lg shadow-black/50"
                        />
                    ) : (
                    ...
                    <div>
                        ...
                        <div className="mt-1 flex flex-wrap items-center gap-2">
                            {character.is_main && (
                                <Pill bgColor="bg-camel-700" textColor="text-camel-200">Main</Pill>
                            )}
                    ...
                    <Can permission="update-characters">
                        <Link
                            ...
                            className="ml-auto flex items-center gap-2 rounded border border-camel-600/60 px-4 py-2 text-sm font-medium text-camel-400 transition-colors hover:bg-camel-600/20"
                        >
                                ...
                                <div className="space-y-2">
                                    {character.linked_characters.map((alt) => (
                                        <Link
                                            ...
                                            className="flex items-center gap-3 rounded border border-forever-600 bg-forever-800/40 px-3 py-2.5 transition-colors hover:border-camel-600/40 hover:bg-forever-800"
                                        >
