...
function AssignmentCell({ side }) {
    ...
    if (side?.type === "target_marker") {
        return (
            <td className="bg-surface/30 px-2">
    ...
    return (
        <td className={`${colorClass || "bg-surface/30"} px-2`}>
        ...
        <table className="w-full table-fixed border-collapse">
            <thead>
                <tr className="sr-only">
                    <th className="sr-only px-2 text-left text-sm font-semibold text-body-muted">Left</th>
                    <th className="sr-only px-2 text-left text-sm font-semibold text-body-muted">Right</th>
                </tr>
            </thead>
...
export default function AssignmentGroup({ group = {}, assignments = [] }) {
    ...
    return (
        <div className="overflow-clip rounded border border-line/30 bg-surface/50 text-center">
            {group.name && (
                <div className="border-b border-line/30 px-4 py-3">
                    <h3 className="text-sm font-semibold text-body-muted">{group.name}</h3>
                </div>
            )}
