...
function AssignmentCell({ side }) {
    ...
    if (side?.type === "target_marker") {
        return (
            <td className="bg-forever-800/30 px-2">
    ...
    return (
        <td className={`${colorClass || "bg-forever-800/30"} px-2`}>
        ...
        <table className="w-full table-fixed border-collapse">
            <thead>
                <tr className="sr-only">
                    <th className="sr-only px-2 text-left text-sm font-semibold text-camel-500">Left</th>
                    <th className="sr-only px-2 text-left text-sm font-semibold text-camel-500">Right</th>
                </tr>
            </thead>
            <tbody className="divide-y divide-camel-600/20">
...
export default function AssignmentGroup({ group = {}, assignments = [] }) {
    ...
    return (
        <div className="overflow-clip rounded border border-camel-600/30 bg-forever-800/50 text-center">
            {group.name && (
                <div className="border-b border-camel-600/30 px-4 py-3">
                    <h3 className="text-sm font-semibold text-camel-500">{group.name}</h3>
                </div>
            )}
