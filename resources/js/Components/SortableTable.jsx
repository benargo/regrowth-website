...
function SortableHeader({ column, label, sortColumn, sortDirection, onSort }) {
    ...
    return (
        <div
            role="columnheader"
            className="table-cell cursor-pointer select-none px-4 py-3 text-left text-sm font-semibold text-camel-500 transition-colors hover:text-camel-400"
    ...
    return (
        <div role="table" className={`table w-full text-left${className ? ` ${className}` : ""}`}>
            <div role="rowgroup" className="table-header-group">
                <div role="row" className="table-row border-b border-forever-600">
                    {columns.map((col) => (
                        <SortableHeader
                            key={col}
                            column={col}
