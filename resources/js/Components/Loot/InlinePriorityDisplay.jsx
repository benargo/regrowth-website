export default function InlinePriorityDisplay({ priorities, weightThreshold = null }) {
    ...
    const visibleWeights = weightThreshold !== null
        ? allWeights.filter((w) => Number(w) < weightThreshold)
        : allWeights;
    ...
    return (
        <span className="flex flex-col items-end gap-1">
            {visibleWeights.map((weight, weightIndex) => (
                <span key={weight} className="flex flex-wrap items-center justify-end gap-x-1 gap-y-0.5">
