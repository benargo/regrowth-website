...
export default function RaidCard({ raid }) {
    const backgroundClass = raid.background ?? "bg-forever-900";
    const borderColor = raid.color ? `#${raid.color}` : null;

    return (
        <Link
            href={route("loot.raids.show", { raid: raid.id, name: raid.slug })}
            className="group block overflow-hidden rounded-lg border border-camel-600/30 shadow-lg transition-all duration-300 hover:scale-[1.03]"
        ...
        >
            <div className={`${backgroundClass} relative aspect-video w-full bg-cover bg-center`}>
                ...
                <div className="absolute right-0 bottom-0 left-0 p-4">
                    {raid.phase_number && (
                        <p className="mb-1 text-xs font-semibold tracking-widest text-camel-400 uppercase">
