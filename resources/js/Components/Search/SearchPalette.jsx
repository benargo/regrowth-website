        ...
        <div className="animate-pulse space-y-1 p-2">
            {[0, 1, 2].map((i) => (
                <div key={i} className="flex items-center gap-4 rounded p-2">
                    <div className="bg-forever-700 h-8 w-8 flex-none rounded" />
                    <div className="flex min-w-0 flex-1 flex-col gap-2">
                        <div className="bg-forever-700 h-3 w-1/3 rounded" />
                        <div className="bg-forever-700 h-4 w-2/3 rounded" />
                    </div>
                </div>
            ))}
        </div>
...
export default function SearchPalette({ open, onClose }) {
    ...
    useEffect(() => {
        ...
        listRef.current
            ?.querySelector(`#search-palette-option-${highlightedIndex}`)
    ...
    return (
        <Dialog open={open} onClose={onClose} initialFocus={inputRef} className="relative z-50">
            ...
            <div className="fixed inset-0 z-10 flex items-start justify-center overflow-y-auto px-4 pt-[10vh] pb-6">
                <DialogPanel
                    transition
                    className="border-primary bg-forever-800 bg-forever-parchment w-full max-w-2xl transform overflow-hidden rounded-lg border shadow-xl transition duration-300 ease-out data-closed:translate-y-4 data-closed:scale-95 data-closed:opacity-0"
                >
                    <div className="flex items-center gap-2 border-b border-camel-600/50 px-4 py-3">
                        ...
                        {raidScope && !scopeDismissed && (
                            <button
                                ...
                                className="inline-flex flex-none items-center gap-1 rounded bg-camel-600/20 px-2 py-1 text-xs font-semibold text-camel-500 hover:bg-camel-600/30"
                            >
                                {raidScope.name}
                    ...
                    {!http.processing && query.trim().length >= MIN_QUERY_LENGTH && total > 0 && (
                        <button
                            type="button"
                            onClick={goToResults}
                            className="w-full border-t border-camel-600/50 px-4 py-2 text-left text-sm text-camel-500 transition-colors hover:bg-camel-600/20"
                        >
