...
class GuildRosterController extends Controller
{
    ...
    public function __construct(
        ...
        $this->classes = Cache::tags(['blizzard', 'mapped-response'])->remember('playable-classes.index', now()->addDays(30), function () {
        ...
        $this->races = Cache::tags(['blizzard', 'mapped-response'])->remember('playable-races.alliance-races', now()->addDays(30), function () {
    ...
    protected function buildMemberCollection()
    {
        return Cache::tags(['blizzard', 'mapped-response'])->remember('guild-roster', now()->addHours(6), function () {
            ...
            return Arr::map(Arr::get($roster, 'members'), function (array $member, string $key) {
                ...
                data_set($member, 'rank', $this->ranks->firstWhere('position', Arr::get($member, 'rank')));
