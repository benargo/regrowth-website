...
| `deprecated`            | Removed features/routes kept only to assert they stay gone — `assertGone()`, 410 responses, retired endpoints.                                                                                                                            |
| `resource`              | Eloquent API Resource and ResourceCollection shape — verifies that a model's `toResource()` / `toResourceCollection()` return the correct Resource/Collection class.                                                                      |
| `laravel-media-library` | Spatie Media Library behaviour — single-file collection enforcement, media attachment, collection naming, and custom properties on media items.                                                                                           |
| `edge-case`             | _Reserved._ Boundary conditions, empty/null states, unusual inputs. Defined for completeness but not currently applied by any suite.                                                                                                      |
